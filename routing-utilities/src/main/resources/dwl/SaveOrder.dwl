...
---
{
	ns0#SaveOrders: {
		...
		ns0#orderSpecDtos: {
			ns1#OrderBaseSpec @(ns2#'type': 'ns1:OrderSpec'): {
				//ns3#Action: vars.action,
				(ns1#CustomProperties: {(vars.configuration.customProperties map ((field) -> {
					ns4#KeyValueOfstringanyType: {
						ns4#Key : field.key,
						ns4#Value @(ns2#'type' : 'ns5:' ++ field.valueType, 'xmlns:ns5' : '$(ns5 as String)'): field.value
					}
				...
				((vars.configuration.fields pluck ($$)) map ((field) -> 
					if ((field as String) == 'TaskSpec') 
				{
					ns1#'$(field)' @(ns2#'type' : 'ns1:' ++ vars.taskSpecType): processValue(vars.configuration.fields[field])
				} 
				else
				{
